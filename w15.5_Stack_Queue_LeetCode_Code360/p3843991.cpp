...
// Topics: 2 Stack, one normal & one Min stack
/*
...
class minStack
{
	...
	public:
		...
		void push(int num)
		{
			...
			// Inserting value in the min stack
			if(minSt.empty()) minSt.push(num);
			else if(minSt.top() >= num) minSt.push(num); 
		}
